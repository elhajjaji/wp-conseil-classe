...
final class CC_Admin {
    ...
    private function require_manage(): void {
        if (!CC_Roles::user_can_manage_plugin()) {
            wp_die(__('Accès refusé.', 'conseil-classe'));
        }
    }
    ...
    public function render_settings(): void {
        ...
        wp_dropdown_pages([
            ...
            'selected' => $loginPageId,
            'show_option_none' => __('— Non défini —', 'conseil-classe'),
        ]);
        echo '</p>';
    ...
    public function handle_save_settings(): void {
        ...
        $data = [
            'nom_etablissement' => sanitize_text_field((string) ($_POST['nom_etablissement'] ?? '')),
            'adresse_etablissement' => CC_Utils::sanitize_textarea($_POST['adresse_etablissement'] ?? ''),
            'telephone_etablissement' => sanitize_text_field((string) ($_POST['telephone_etablissement'] ?? '')),
            'email_etablissement' => sanitize_email((string) ($_POST['email_etablissement'] ?? '')),
            'site_web_etablissement' => esc_url_raw((string) ($_POST['site_web_etablissement'] ?? '')),
            'nom_directeur' => sanitize_text_field((string) ($_POST['nom_directeur'] ?? '')),
            'nom_principal' => sanitize_text_field((string) ($_POST['nom_principal'] ?? '')),
            'nom_association_parents' => sanitize_text_field((string) ($_POST['nom_association_parents'] ?? '')),
            'adresse_association_parents' => CC_Utils::sanitize_textarea($_POST['adresse_association_parents'] ?? ''),
            'telephone_association_parents' => sanitize_text_field((string) ($_POST['telephone_association_parents'] ?? '')),
            'email_association_parents' => sanitize_email((string) ($_POST['email_association_parents'] ?? '')),
            'site_web_association_parents' => esc_url_raw((string) ($_POST['site_web_association_parents'] ?? '')),
            'president_association' => sanitize_text_field((string) ($_POST['president_association'] ?? '')),
            'vice_president_association' => sanitize_text_field((string) ($_POST['vice_president_association'] ?? '')),
            'secretaire_association' => sanitize_text_field((string) ($_POST['secretaire_association'] ?? '')),
            'tresorier_association' => sanitize_text_field((string) ($_POST['tresorier_association'] ?? '')),
            'max_parents_per_conseil' => max(1, (int) ($_POST['max_parents_per_conseil'] ?? 2)),
        ];

        CC_Repo::update_settings($data);

        update_option('cc_parent_login_page_id', (int) ($_POST['cc_parent_login_page_id'] ?? 0));
        update_option('cc_plannings_page_id', (int) ($_POST['cc_plannings_page_id'] ?? 0));
        update_option('cc_my_councils_page_id', (int) ($_POST['cc_my_councils_page_id'] ?? 0));
        update_option('cc_report_form_page_id', (int) ($_POST['cc_report_form_page_id'] ?? 0));
    ...
    public function handle_year_create(): void {
        ...
        if ($nom !== '') {
            CC_Repo::create_year($nom, !empty($_POST['active']));
        }
    ...
    public function render_classes(): void {
        ...
        echo '</p>';
        if (!empty($_GET['imported']) || !empty($_GET['updated']) || !empty($_GET['import_errors'])) {
            echo '<div class="notice notice-success"><p>' . esc_html(sprintf(
                __('Import classes terminé: %d créé(s), %d mis à jour, %d erreur(s).', 'conseil-classe'),
                (int) ($_GET['imported'] ?? 0),
                (int) ($_GET['updated'] ?? 0),
                (int) ($_GET['import_errors'] ?? 0)
            )) . '</p></div>';
        }
        echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" enctype="multipart/form-data">';
    ...
    public function handle_class_create(): void {
        ...
        $nom = sanitize_text_field((string) ($_POST['nom'] ?? ''));
        $niveau = sanitize_text_field((string) ($_POST['niveau'] ?? ''));
        if ($nom !== '' && $niveau !== '') {
    ...
    public function render_councils(): void {
        ...
        echo '</p>';
        if (!empty($_GET['imported']) || !empty($_GET['ignored']) || !empty($_GET['import_errors'])) {
            echo '<div class="notice notice-success"><p>' . esc_html(sprintf(
                __('Import conseils terminé: %d créé(s), %d ignoré(s), %d erreur(s).', 'conseil-classe'),
                (int) ($_GET['imported'] ?? 0),
                (int) ($_GET['ignored'] ?? 0),
                (int) ($_GET['import_errors'] ?? 0)
            )) . '</p></div>';
    ...
    public function handle_settings_import_csv(): void {
        ...
        $delimiter = ',';
        $handle = $this->csv_open_import_handle((string) $_FILES['csv_file']['tmp_name'], $delimiter);
        if (!$handle) {
            $this->redirect_admin('cc_settings');
        }
    ...
    public function handle_classes_export_csv(): void {
        ...
        $year = CC_Repo::get_active_year();
        if (!$year) {
            wp_die(__('Aucune année active.', 'conseil-classe'));
        }
        $classes = CC_Repo::list_classes_for_year((int) $year['id']);
    ...
    public function handle_councils_export_csv(): void {
        ...
        $term = CC_Repo::get_active_term();
        if (!$year || !$term) {
            wp_die(__('Année/trimestre actif manquant.', 'conseil-classe'));
        }
        $councils = CC_Repo::list_councils((int) $year['id'], (int) $term['id']);
    ...
    public function handle_council_create(): void {
        ...
        $date = sanitize_text_field((string) ($_POST['date_conseil'] ?? ''));
        $hd = sanitize_text_field((string) ($_POST['heure_debut'] ?? ''));
        $hf = sanitize_text_field((string) ($_POST['heure_fin'] ?? ''));
        $salle = sanitize_text_field((string) ($_POST['salle_conseil'] ?? ''));
        $president = sanitize_text_field((string) ($_POST['president_conseil'] ?? ''));
    ...
    public function render_parents(): void {
        ...
        $profile = sanitize_key((string) ($_GET['cc_profile'] ?? 'all'));
        ...
        if (!empty($_GET['imported']) || !empty($_GET['import_updated']) || isset($_GET['import_errors'])) {
            $imported = (int) ($_GET['imported'] ?? 0);
            $importUpdated = (int) ($_GET['import_updated'] ?? 0);
            $errors = (int) ($_GET['import_errors'] ?? 0);
            $suffix = (!empty($_GET['import_mode']) && (string) $_GET['import_mode'] === 'admins')
                ? ' ' . __('(import équipe conseil)', 'conseil-classe')
                : '';
            echo '<div class="notice notice-success"><p>' . esc_html(
                sprintf(
                    /* translators: %s: optional suffix, %1$d: créés, %2$d: mis à jour, %3$d: erreurs */
                    __('Import terminé%s : %1$d créé(s), %2$d mis à jour, %3$d erreur(s).', 'conseil-classe'),
                    $suffix,
                    $imported,
                    $importUpdated,
                    $errors
...
function ccAskParentPassword(form) {
  ...
  return true;
    ...
    public function handle_parent_create(): void {
        ...
        $nom = sanitize_text_field((string) ($_POST['nom'] ?? ''));
        $prenom = sanitize_text_field((string) ($_POST['prenom'] ?? ''));
        $email = CC_Utils::normalize_email((string) ($_POST['email'] ?? ''));
        $telephone = sanitize_text_field((string) ($_POST['telephone'] ?? ''));

        $wpRole = sanitize_key((string) ($_POST['cc_new_wp_role'] ?? CC_Roles::ROLE_PARENT));
        ...
        $plainPw = $this->resolve_parent_password_plain((string) ($_POST['code_acces'] ?? ''));
    ...
    public function handle_parent_regenerate_code(): void {
        ...
        if ($id > 0) {
            ...
            if ($row) {
                $custom = trim(sanitize_text_field((string) ($_POST['code_acces_custom'] ?? '')));
                if ($custom !== '') {
                    if (strlen($custom) < 6) {
                        $this->redirect_admin('cc_parents', ['pw_err' => '1']);
                    }
    ...
    public function handle_parents_export_csv(): void {
        ...
        $profile = sanitize_key((string) ($_REQUEST['cc_profile'] ?? 'all'));
        if (!in_array($profile, ['all', 'parent', 'admin'], true)) {
            $profile = 'all';
        }
    ...
    public function handle_parents_import_csv(): void {
        ...
        [$created, $updated, $errors] = $this->consume_parents_csv_import($_FILES['csv_file']['tmp_name'], 'parent');
    ...
    public function handle_parents_import_admins_csv(): void {
        ...
        if (empty($_FILES['csv_file_admins']) || !is_array($_FILES['csv_file_admins']) || empty($_FILES['csv_file_admins']['tmp_name'])) {
            $this->redirect_admin('cc_parents');
        }

        [$created, $updated, $errors] = $this->consume_parents_csv_import($_FILES['csv_file_admins']['tmp_name'], 'admin');
    ...
    public function render_registrations(): void {
        ...
        $year = CC_Repo::get_active_year();
        $term = CC_Repo::get_active_term();
        $councilId = (int) ($_GET['council_id'] ?? 0);
        ...
        foreach ($councils as $co) {
            ...
            $selected = $councilId === (int) $co['id'] ? ' selected' : '';
            echo '<option value="' . esc_attr((string) $co['id']) . '"' . $selected . '>' . esc_html($label) . '</option>';
        }
        echo '</select> ';
    ...
    public function handle_report_update(): void {
        ...
        CC_Repo::update_report_content($id, [
            'nom_parent' => sanitize_text_field((string) ($_POST['nom_parent'] ?? '')),
            'prenom_parent' => sanitize_text_field((string) ($_POST['prenom_parent'] ?? '')),
            'email_parent' => sanitize_email((string) ($_POST['email_parent'] ?? '')),
            'profs_participants' => CC_Utils::sanitize_textarea($_POST['profs_participants'] ?? ''),
            'delegue_eleve_1_nom' => sanitize_text_field((string) ($_POST['delegue_eleve_1_nom'] ?? '')),
            'delegue_eleve_1_prenom' => sanitize_text_field((string) ($_POST['delegue_eleve_1_prenom'] ?? '')),
            'delegue_eleve_2_nom' => sanitize_text_field((string) ($_POST['delegue_eleve_2_nom'] ?? '')),
            'delegue_eleve_2_prenom' => sanitize_text_field((string) ($_POST['delegue_eleve_2_prenom'] ?? '')),
            'delegue_parent_1_nom' => sanitize_text_field((string) ($_POST['delegue_parent_1_nom'] ?? '')),
            'delegue_parent_1_prenom' => sanitize_text_field((string) ($_POST['delegue_parent_1_prenom'] ?? '')),
            'delegue_parent_2_nom' => sanitize_text_field((string) ($_POST['delegue_parent_2_nom'] ?? '')),
            'delegue_parent_2_prenom' => sanitize_text_field((string) ($_POST['delegue_parent_2_prenom'] ?? '')),
            'remarque_principal' => CC_Utils::sanitize_textarea($_POST['remarque_principal'] ?? ''),
            'remarque_prof_principal' => CC_Utils::sanitize_textarea($_POST['remarque_prof_principal'] ?? ''),
            'remarques_autres_profs' => CC_Utils::sanitize_textarea($_POST['remarques_autres_profs'] ?? ''),
            'remarques_eleves_delegues' => CC_Utils::sanitize_textarea($_POST['remarques_eleves_delegues'] ?? ''),
            'remarques_parents' => CC_Utils::sanitize_textarea($_POST['remarques_parents'] ?? ''),
            'nb_felicitations' => max(0, (int) ($_POST['nb_felicitations'] ?? 0)),
            'nb_encouragements' => max(0, (int) ($_POST['nb_encouragements'] ?? 0)),
            'nb_compliments' => max(0, (int) ($_POST['nb_compliments'] ?? 0)),
            'nb_mise_en_garde_travail' => max(0, (int) ($_POST['nb_mise_en_garde_travail'] ?? 0)),
            'nb_mise_en_garde_comportement' => max(0, (int) ($_POST['nb_mise_en_garde_comportement'] ?? 0)),
        ]);
    ...
    public function handle_report_export(): void {
        ...
        $id = (int) ($_GET['id'] ?? 0);
        $format = sanitize_text_field((string) ($_GET['format'] ?? 'pdf'));
        $report = $id > 0 ? CC_Repo::get_report($id) : null;
        if (!$report) {
            wp_die(__('Compte-rendu introuvable.', 'conseil-classe'));
        }
    ...
    private function emit_report_pdf_browser(string $fragment, string $filenameBase, string $title): void {
        ...
        echo '<script src="' . esc_url($scriptHref) . '"></script>';
        echo '<script>';
    ...
    public function render_pdf_templates(): void {
        ...
        $checked = ((int) ($template['actif'] ?? 0) === 1) ? ' checked' : '';
        echo '<p><label><input type="checkbox" name="actif" value="1"' . $checked . ' /> ' . esc_html__('Définir actif', 'conseil-classe') . '</label></p>';
    ...
    public function handle_pdf_template_save(): void {
        ...
        $html = (string) ($_POST['html_template'] ?? '');
        $css = (string) ($_POST['css_style'] ?? '');
    ...
    public function render_logs(): void {
        ...
        $rows = (array) $wpdb->get_results("SELECT * FROM $logs ORDER BY timestamp DESC LIMIT 100", ARRAY_A);
