...
final class CC_Public {
    ...
    public function gate_conseil_pages(): void {
        ...
        if (!CC_Roles::user_can_parent_portal()) {
            wp_die(
                ...
                __('Accès refusé', 'conseil-classe'),
                ['response' => 403]
            );
        }
    }
    ...
    public function shortcode_plannings($atts): string {
        ...
        echo '<h3>' . esc_html(sprintf(__('Planning des conseils (%s - %s)', 'conseil-classe'), $year['nom'], $term['nom'])) . '</h3>';
    ...
    public function handle_parent_register(): void {
        ...
        if ($councilId <= 0) {
            wp_die(__('Conseil invalide.', 'conseil-classe'));
        }
        ...
        if ($count >= $maxParents) {
            wp_die(sprintf(__('Limite atteinte : maximum %d parent(s) autorisé(s) par conseil.', 'conseil-classe'), $maxParents));
        }
    ...
    public function shortcode_my_councils($atts): string {
        ...
        echo $this->render_parent_councils_table($actifs, $reportFormUrl);

        echo '<h3 style="margin-top:24px">' . esc_html__('Historique', 'conseil-classe') . '</h3>';
        echo $this->render_parent_councils_table($hist, $reportFormUrl);
        echo '</div>';
    ...
    public function handle_report_create(): void {
        ...
        if (!CC_Repo::is_parent_registered($councilId, (int) $parent['id'])) {
            wp_die(__('Vous n’êtes pas inscrit(e) à ce conseil.', 'conseil-classe'));
        }

        if (CC_Repo::get_report_for_council($councilId)) {
            wp_die(__('Un compte-rendu existe déjà.', 'conseil-classe'));
        }
